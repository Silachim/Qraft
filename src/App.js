...
function genId(len = 7) { return Math.random().toString(36).slice(2, 2 + len); }
...
// ── Toggle ────────────────────────────────────────────────────────────────────
function Toggle({value,onChange,dm}){
  return(
    <button onClick={()=>onChange(!value)} className={`w-12 h-6 rounded-full relative flex-shrink-0 transition-colors duration-300 ${value?dm?"bg-cyan-500":"bg-purple-500":dm?"bg-gray-700":"bg-gray-300"}`}>
...
function AuthModal({dm,onClose}){
  ...
  const handleEmail=async()=>{
    ...
    try{if(mode==="login")await signInWithEmailAndPassword(auth,email,password);else await createUserWithEmailAndPassword(auth,email,password);onClose();}
  ...
  return(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className={`${card} rounded-3xl p-6 w-full max-w-sm shadow-2xl`} onClick={e=>e.stopPropagation()}>
        ...
        <button onClick={handleEmail} disabled={loading} className="w-full py-3 rounded-xl font-bold text-white mb-3 disabled:opacity-50" style={{background:"linear-gradient(135deg,#00f5ff,#a855f7)"}}>
          {loading?"Please wait…":mode==="login"?"Sign In":"Create Account"}
        </button>
        <button onClick={handleGoogle} disabled={loading} className={`w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 mb-4 ${dm?"bg-white/10 text-white hover:bg-white/20 border border-white/10":"bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
...
function Dashboard({user,dm,onClose,onEdit}){
  ...
  const fetchCodes=useCallback(async()=>{
    setLoading(true);
    try{const q=query(collection(db,"qrcodes"),where("uid","==",user.uid),orderBy("createdAt","desc"));const snap=await getDocs(q);setQrCodes(snap.docs.map(d=>({id:d.id,...d.data()})));}
    catch(e){console.error(e);}
    finally{setLoading(false);}
  },[user.uid]);
  useEffect(()=>{fetchCodes();},[fetchCodes]);
  const saveEdit=async()=>{
    if(!editUrl.trim())return;setSaving(true);
    try{await updateDoc(doc(db,"qrcodes",editId),{destination:editUrl.startsWith("http")?editUrl:"https://"+editUrl,label:editLabel,updatedAt:serverTimestamp()});setQrCodes(prev=>prev.map(q=>q.id===editId?{...q,destination:editUrl,label:editLabel}:q));setEditId(null);}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-3 sm:p-6" onClick={onClose}>
      <div className={`${card} rounded-3xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col`} onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b" style={{borderColor:dm?"rgba(255,255,255,0.1)":"#e5e7eb"}}>
          <div><h2 className={`text-xl font-black ${txt}`}>My QR Codes</h2><p className={`text-xs mt-0.5 ${sub}`}>{user.email}</p></div>
          <div className="flex items-center gap-2">
            <button onClick={fetchCodes} className={`p-2 rounded-xl ${dm?"bg-white/5 hover:bg-white/10 text-gray-400":"bg-gray-100 hover:bg-gray-200 text-gray-600"}`}><RefreshCw className="w-4 h-4"/></button>
            <button onClick={onClose} className={`p-2 rounded-xl ${dm?"bg-white/5 hover:bg-red-500/20 text-gray-400 hover:text-red-400":"bg-gray-100 hover:bg-red-50 text-gray-600 hover:text-red-500"}`}><X className="w-4 h-4"/></button>
          </div>
        </div>
        <div className="overflow-y-auto flex-1 p-4 space-y-3">
          {loading?(
            <div className="text-center py-12"><div className={`w-8 h-8 border-2 border-t-transparent rounded-full animate-spin mx-auto mb-3 ${dm?"border-cyan-400":"border-purple-500"}`}/><p className={`text-sm ${sub}`}>Loading…</p></div>
          ):qrCodes.length===0?(
            <div className="text-center py-12"><QrCode className={`w-12 h-12 mx-auto mb-3 ${dm?"text-gray-700":"text-gray-300"}`}/><p className={`text-sm font-semibold ${txt}`}>No dynamic QR codes yet</p><p className={`text-xs mt-1 ${sub}`}>Enable "Dynamic QR" in the main form to create one</p></div>
          ):qrCodes.map(qr=>(
            <div key={qr.id} className={`${rowBg} rounded-2xl p-4`}>
              {editId===qr.id?(
                <div className="space-y-2">
                  <input value={editLabel} onChange={e=>setEditLabel(e.target.value)} placeholder="Label" className={inp}/>
                  <input value={editUrl} onChange={e=>setEditUrl(e.target.value)} placeholder="New destination URL" className={inp}/>
                  <div className="flex gap-2">
                    <button onClick={saveEdit} disabled={saving} className="flex-1 py-2 rounded-xl text-sm font-bold text-white disabled:opacity-50" style={{background:"linear-gradient(135deg,#00f5ff,#a855f7)"}}>{saving?"Saving…":"Save"}</button>
                    <button onClick={()=>setEditId(null)} className={`px-4 py-2 rounded-xl text-sm ${dm?"bg-white/5 text-gray-300":"bg-gray-100 text-gray-600"}`}>Cancel</button>
                  </div>
                </div>
              ):(
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${dm?"bg-cyan-400/20 text-cyan-400":"bg-purple-100 text-purple-600"}`}>dynamic</span>
                    </div>
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={()=>{setEditId(qr.id);setEditUrl(qr.destination);setEditLabel(qr.label||"");}} className={`p-1.5 rounded-lg ${dm?"hover:bg-cyan-400/20 text-gray-400 hover:text-cyan-400":"hover:bg-purple-50 text-gray-400 hover:text-purple-600"}`}><Edit2 className="w-3.5 h-3.5"/></button>
                    <button onClick={()=>onEdit(qr)} className={`p-1.5 rounded-lg ${dm?"hover:bg-white/10 text-gray-400 hover:text-white":"hover:bg-gray-100 text-gray-400 hover:text-gray-700"}`}><Download className="w-3.5 h-3.5"/></button>
                    <button onClick={()=>deleteCode(qr.id)} disabled={deleting===qr.id} className={`p-1.5 rounded-lg ${dm?"hover:bg-red-500/20 text-gray-400 hover:text-red-400":"hover:bg-red-50 text-gray-400 hover:text-red-500"}`}>
        ...
        <div className="p-4 border-t text-center" style={{borderColor:dm?"rgba(255,255,255,0.1)":"#e5e7eb"}}>
          <p className={`text-xs ${sub}`}>Edit destination anytime without reprinting.</p>
        </div>
      </div>
    </div>
  );
...
function FullscreenQR({qrData,qrColor,qrBg,qrShape,embedLogo,onClose}){
  ...
  useEffect(()=>{
    ...
    getQRMatrix(qrData,320).then(({modules})=>{
      ...
      if(embedLogo){const ctx=el.getContext("2d"),img=new Image();img.src=logo;img.onload=()=>{const ls=320*0.18,x=(el.width-ls)/2,y=(el.height-ls)/2;ctx.fillStyle=qrBg;ctx.fillRect(x-5,y-5,ls+10,ls+10);ctx.drawImage(img,x,y,ls,ls);};}
    });
  ...
  return(
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/95 backdrop-blur-sm" onClick={onClose}>
      <div className="p-8 rounded-3xl" style={{background:qrBg}}><canvas ref={canvasRef}/></div>
...
export default function QRaft(){
  ...
  const shortId=rIdx!==-1?parts[rIdx+1]||null:null;
  if(shortId)return <Redirect shortId={shortId}/>;
  return(<><ToastProvider/><QRaftApp/></>);
}
...
function QRaftApp(){
  ...
  const [shareMsg,      setShareMsg]      = useState(""); // eslint-disable-line no-unused-vars
  ...
  // ── ALL useCallback / useEffect ────────────────────────────────────────────
  const fetchQrCodes = useCallback(async(uid)=>{
    try{const q=query(collection(db,"qrcodes"),where("uid","==",uid),orderBy("createdAt","desc"));const snap=await getDocs(q);setQrCodes(snap.docs.map(d=>({id:d.id,...d.data()})));}
    catch(e){console.error(e);}
  },[]);
  ...
  const drawQR=useCallback(async(text,container)=>{
    ...
    try{
      ...
      if(embedLogo){const ctx=canvas.getContext("2d"),img=new Image();img.src=logo;img.onload=()=>{const ls=qrSize*0.18,x=(canvas.width-ls)/2,y=(canvas.height-ls)/2;ctx.fillStyle=qrBg;ctx.fillRect(x-5,y-5,ls+10,ls+10);ctx.drawImage(img,x,y,ls,ls);};}
    }catch{
      await loadQRious();
      const canvas=document.createElement("canvas");
      container.appendChild(canvas);
  ...
  useEffect(()=>{
    ...
    if(useExpiry&&expiryDate&&new Date()>new Date(expiryDate)){setExpired(true);setQrData("");if(qrRef.current)qrRef.current.innerHTML="";return;}
    setExpired(false);
  ...
  },[buildData,drawQR,useExpiry,expiryDate,usePassword,qrPassword,activeTab,history,scanCount]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const applyTemplate=t=>{
    setQrColor(t.fg);setQrBg(t.bg);setQrShape(t.shape);setActiveTemplate(t.id);
    const raw=buildData();
    if(raw){const final=usePassword&&qrPassword?`${raw}|PWD:${qrPassword}`:raw;setTimeout(()=>drawQR(final,qrRef.current),50);}
  ...
  const saveDynamicQR=async()=>{
    ...
    const raw=buildData();if(!raw){toast.error("Please fill in the form first.");return;}
  ...
  const handleGetStarted=()=>{localStorage.setItem("qraft_visited","1");setShowLanding(false);};
  if(showLanding)return(<Landing dm={darkMode} onGetStarted={handleGetStarted} onToggleDark={()=>setDarkMode(!darkMode)}/>);
    ...
    <div className={`min-h-screen ${bg} transition-colors duration-300`}>
        ...
        <div className="flex items-center justify-between mb-2 sm:mb-4">
          ...
          <div className="flex items-center gap-2">
            {user?(
              ...
              <button onClick={()=>setShowAuth(true)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${dm?"bg-cyan-400/10 text-cyan-400 border border-cyan-400/20":"bg-purple-50 text-purple-600 border border-purple-200"}`}><LogIn className="w-3 h-3"/>Sign In</button>
            )}
            <button onClick={()=>setDarkMode(!dm)} className={`p-2 rounded-full ${dm?"bg-[#1a1a2e] text-yellow-400 border border-yellow-400/20":"bg-white text-gray-600 shadow"}`}>
        ...
        <div className={`${card} rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden`} style={dm?{boxShadow:"0 0 60px #00f5ff0d,0 25px 50px #00000090"}:{}}>
          <div className={`hidden sm:flex border-b ${dm?"border-white/10":"border-gray-200"} overflow-x-auto`}>
            {TABS.map(({id,label,icon:Icon})=>(
              <button key={id} onClick={()=>setActiveTab(id)} className={`flex-1 min-w-[80px] flex items-center justify-center gap-1.5 py-3 sm:py-4 text-xs sm:text-sm font-medium transition-all whitespace-nowrap px-2 ${activeTab===id?dm?"text-cyan-400 border-b-2 border-cyan-400 bg-cyan-400/5":"text-purple-600 border-b-2 border-purple-600 bg-purple-50":dm?"text-gray-500 hover:text-gray-300 hover:bg-white/5":"text-gray-500 hover:text-gray-700 hover:bg-gray-50"}`}>
          ...
          <div className="p-4 sm:p-6 lg:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-10">
              ...
              <div className="space-y-4">
                ...
                {activeTab==="url"&&(<div><label className={lbl}>Website URL</label><input type="text" value={urlInput} onChange={e=>setUrlInput(e.target.value)} placeholder="e.g. google.com" className={inp}/><p className={`text-sm mt-1 ${sub}`}>https:// added automatically if omitted.</p></div>)}
                ...
                <div className={`rounded-2xl p-4 space-y-3 ${panelBg}`}>
                  ...
                  {isDynamic&&(
                    <>
                      <p className={`text-xs ${sub}`}>Dynamic QR codes can be edited after printing.</p>
                      ...
                      {savedDynamic?(
                        <div className={`text-xs px-3 py-2 rounded-xl space-y-1 ${dm?"bg-green-500/10 border border-green-500/20 text-green-300":"bg-green-50 border border-green-200 text-green-700"}`}>
                          ...
                          <p className="opacity-60">Edit anytime from My QR Codes.</p>
                        </div>
                      ):(
                        <button onClick={saveDynamicQR} disabled={savingDynamic} className="w-full py-2.5 rounded-xl text-sm font-bold text-white disabled:opacity-50 flex items-center justify-center gap-2" style={{background:"linear-gradient(135deg,#00f5ff,#a855f7)"}}>
                ...
                <div className={`relative w-full rounded-2xl p-4 sm:p-6 flex items-center justify-center min-h-[260px] sm:min-h-[300px] ${dm?"bg-[#1a1a2e] border border-white/5":"bg-gray-50 border border-gray-200"}`} style={dm&&qrData?{boxShadow:`0 0 50px ${qrColor}28`}:{}}>
                  {qrData?(
                    <div className="text-center">
                      ...
                      {isDynamic&&savedDynamic&&<div className={`mt-2 text-[10px] font-semibold px-2 py-1 rounded-full inline-block ${dm?"bg-cyan-400/20 text-cyan-400":"bg-purple-100 text-purple-600"}`}>⚡ Dynamic — editable anytime</div>}
      ...
      <div className={`fixed bottom-0 left-0 right-0 sm:hidden z-40 backdrop-blur-md ${dm?"bg-[#12121f]/95 border-t border-white/10":"bg-white/95 border-t border-gray-200"}`}>
        <nav className="flex justify-around px-1 py-1.5">
          {TABS.map(({id,label,icon:Icon})=>(
            <button key={id} onClick={()=>setActiveTab(id)} className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all duration-200 relative ${activeTab===id?dm?"text-cyan-400 bg-cyan-400/10":"text-purple-600 bg-purple-50":dm?"text-gray-600":"text-gray-400"}`}>
              <Icon className="w-5 h-5"/>
              <span className="text-[9px] font-semibold">{label}</span>
              {activeTab===id&&<span className={`absolute -top-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${dm?"bg-cyan-400":"bg-purple-600"}`}/>}
            </button>
          ))}
        </nav>
      </div>
